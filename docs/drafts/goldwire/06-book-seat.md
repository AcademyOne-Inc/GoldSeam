# Call 6 — Book the seat

[← Call 5 — GoldCard authorize](05-goldcard-authorize.md) · [Summary](README.md) · Next: [Call 7 — Get a booking →](07-get-booking.md)
...
- **Before:** [Call 3](03-hold-seat.md) held `hld_01J8Z3XQ2K`; [Call 5](05-goldcard-authorize.md) authorized $241.00 as `auth_7HF2Q9`.
- **This call:** "Register me."
- **After:** show the confirmation. Check back any time with [Call 7 — Get a booking](07-get-booking.md). Drop with [Call 8](08-release-seat.md).
...
| `goldcard_authorization_id` | `auth_7HF2Q9` | **yes for prepay** | the money, from Call 5 | must be for this hold and the full amount due now |
| `goldcard_guarantee_id` | *(not sent)* | **yes for reserve** | the guarantee, from Call 5 | send one or the other, never both |
| `school_student_ref` | `A00482913` | only if the school needs it | the learner's student ID **at the school** | sent to the school only; stored encrypted; **never returned in any reply** |
...
4. Checks the attestations and, if the school needs it, the student reference.
5. Records the booking as `SUBMITTED` and marks the hold `BOOKED`.
6. Sends the registration to the school's system, waiting up to 8 seconds: term `2027SP`, CRN `21457`, student `A00482913`, GoldWire booking `bkg_01J8Z42M7R`.
...
| `booking_id` | `bkg_01J8Z42M7R` | **the booking — use it in Calls 7 and 8** |
...
```json
{
  ...
  "refund_policy": { "full_refund_until": "2027-02-02", "source": "pol_225070_refunds_2026_27" },
  "s3": {
    "booking": { "key": "goldwire/225070/2027SP/sec_225070_2027SP_ENGL1301_002/bookings/bkg_01J8Z42M7R.json", "etag": "\"f5e39d0e2b7c41a5f6\"", "version_id": "7Jk2mNpQ4rStUvWx.yZ1aB3cD5eF7gH9" }
  }
}
```
...
| 400 | `VALIDATION_FAILED` | `goldcard_authorization_id is required when payment_mode is prepay.` | | send the id from Call 5 |
| 400 | `VALIDATION_FAILED` | `goldcard_guarantee_id is required when payment_mode is reserve.` | | send the id from Call 5 |
...
| 400 | `VALIDATION_FAILED` | `Unknown field 'section_id'. book_seat accepts learner_id, hold_id, payment_mode, goldcard_authorization_id, goldcard_guarantee_id, school_student_ref, attestations.` | | drop it — the section comes from the hold |
...
| 404 | `AUTHORIZATION_NOT_FOUND` | `GoldCard has no authorization auth_7HF2Q0 for learner lrn_8f3a2c91d7.` | typo, or authorized for someone else | authorize again (Call 5) |
| 404 | `GUARANTEE_NOT_FOUND` | `GoldCard has no guarantee gtd_4KX81N for learner lrn_8f3a2c91d7.` | | Call 5 again |
| 409 | `HOLD_ALREADY_BOOKED` | `Hold hld_01J8Z3XQ2K was already booked as bkg_01J8Z42M7R on 2026-09-25T14:11:53Z.` | booked before under a different key | read that booking (Call 7) |
...
| 410 | `HOLD_EXPIRED` | `Hold hld_01J8Z3XQ2K expired at 2026-09-25T14:24:05Z. The seat was released. Hold a seat again.` | more than 20 minutes | Call 3 again; release the old authorization |
| 410 | `AUTHORIZATION_EXPIRED` | `Authorization auth_7HF2Q9 expired at 2026-10-02T14:06:30Z. Authorize again with GoldCard.` | | Call 5 again |
...
| 422 | `STUDENT_REF_REQUIRED` | `School 225070 requires the learner's student ID there (school_student_ref) to register.` | | ask the learner for it |
...
2. `goldwire/225070/2027SP/sec_225070_2027SP_ENGL1301_002/bookings/bkg_01J8Z42M7R.json` — created as `SUBMITTED` (`If-None-Match: *`); the school's answer is a **new version** (`If-Match` on the version just written). The student reference is stored encrypted inside it.
